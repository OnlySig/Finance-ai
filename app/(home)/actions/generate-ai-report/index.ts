...
const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  if (!isProPlan) return redirect("/subscription");
  const openAi = new OpenIa({
  ...
  const getDataForMonth = await db.transaction.findMany({
    where,
  });
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar minha vida financeira. As transações estão por ponto e vírgula. A estrutura de cada uma é {DATA}---{VALOR}---{TIPO}---{CATEGORIA}. São elas:
  ${getDataForMonth
    .map(
      (transaction) =>
